...
describe("getWikibaseItems", () => {
  testWikidataItemFiles.forEach(testWikidataItemFile => {
    it(`should successfully parse the file ${testWikidataItemFile.qid}`, () => {
      ...
      expect(fileItem!.articles).to.have.length(
        testWikidataItemFile.articlesCount
      );
      expect(fileItem!.prefLabel).to.not.be.empty;
      expect(fileItem!.statements).to.have.length(
        testWikidataItemFile.directClaimsCount +
          testWikidataItemFile.fullStatementsCount
      );
      expect(
        fileItem!.statements.filter(statement => statement.type === "Direct")
      ).to.have.length(testWikidataItemFile.directClaimsCount);
      expect(
        fileItem!.statements.filter(statement => statement.type === "Full")
      ).to.have.length(testWikidataItemFile.fullStatementsCount);
    });
  });
});
