...
@pytest.mark.parametrize("wikimedia_commons_image_url", wikimedia_commons_image_urls)
def test_enrich_schema_image_object(
    data_dir_path: Path, wikimedia_commons_image_url: str
):
    unenriched_image = (
        SchemaImageObject.builder(
            uri=URIRef("http://example.com/Image"),
        )
        .set_src(URIRef(wikimedia_commons_image_url))
        .build()
    )
...
@pytest.mark.parametrize("wikimedia_commons_image_url", wikimedia_commons_image_urls)
def test_enrich_stub_image(data_dir_path: Path, wikimedia_commons_image_url: str):
    unenriched_image = StubImage(URIRef(wikimedia_commons_image_url))
    enriched_models = tuple(
        WikimediaCommonsEnricher(
            cache_dir_path=data_dir_path / "test" / "wikimedia_commons"
        )((unenriched_image,))
    ...
    enriched_image = enriched_models[0]
    assert isinstance(enriched_image, Image)
    assert not isinstance(enriched_image, StubImage)
    assert enriched_image.uri == unenriched_image.uri
    assert enriched_image.creators
    assert enriched_image.licenses
    # assert (
