...
def test_transform(data_dir_path: Path):
    ...
    models = tuple(
        DirectoryTransformer(
            # collection_uri="urn:paradicms:etl:pipeline:test:collection:default",
    ...
    assert len(works) == 2
    work1 = works[
        URIRef("urn:paradicms:etl:pipeline:test:schema-creative-work:test_work1")
    ]
    work2 = works[
        URIRef("urn:paradicms:etl:pipeline:test:schema-creative-work:test_work2")
    ]

    assert work2.image_uris == (
        URIRef("urn:paradicms:etl:pipeline:test:schema-image-object:test_work2"),
    )
    ...
    assert work1_description.rights_holders == (
        URIRef("urn:paradicms:etl:pipeline:test:foaf-person:test_person"),
    )
