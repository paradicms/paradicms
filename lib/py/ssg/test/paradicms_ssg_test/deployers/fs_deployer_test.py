...
def test_archive_rename(tmp_path):
    deploy_dir_path = tmp_path / "deploy"
    deploy_dir_path.mkdir()
    app_out_dir_path = tmp_path / "out"
    ...
    sut(app_out_dir_path=app_out_dir_path)
    ...
    assert deploy_dir_path_subdirs[0].startswith("pre-"), deploy_dir_path_subdirs
