...
class TestDataTransformer(_Transformer):
    ...
    def __generate_object(
        ...
        *,
        ...
        properties.extend(
            object_property_values(self.__LANGUAGES, 2, PropertyDefinitions.LANGUAGE)
        )
        properties.extend(
            object_property_values(self.__MATERIALS, 2, PropertyDefinitions.MATERIAL)
        )
        properties.extend(
        ...
        properties.extend(
            Property(
                key=PropertyDefinitions.PROVENANCE.key, value=f"{title} provenance {i}"
            )
            for i in range(2)
        )
        properties.extend(
            object_property_values(self.__PUBLISHERS, 2, PropertyDefinitions.PUBLISHER)
        )
        properties.extend(
        ...
        properties.extend(
            object_property_values(self.__TECHNIQUES, 2, PropertyDefinitions.TECHNIQUE)
        )
        properties.extend(
            object_property_values(self.__TEMPORALS, 2, PropertyDefinitions.TEMPORAL)
        )
        properties.extend(
