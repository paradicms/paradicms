    ...
    def deploy(self, *, gui_out_dir_path: Path) -> None:
        for dir_path, sub_dir_names, file_names in os.walk(gui_out_dir_path):
            for file_name in file_names:
                ...
                key = str(file_path.relative_to(gui_out_dir_path)).replace(
                    os.path.sep, "/"
                )

                if not self.__force_upload:
                    try:
                        self.__s3_client.head_object(
                            Bucket=self.__s3_bucket_name, Key=key
                        )
                        self._logger.debug(
                ...
                guess_mime_type, _ = mimetypes.guess_type(
                    file_path.as_uri(), strict=False
                )
                if guess_mime_type is None:
                    raise ValueError(
                        f"unable to guess MIME type for the file {file_path}"
                    )
